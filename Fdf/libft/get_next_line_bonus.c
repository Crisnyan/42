...
/*   Updated: 2024/03/01 15:24:40 by cmanica-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*update_storage(char *storage)
{
	...
	i++;
	new_storage = malloc(sizeof (char) * ft_gnl_strlen(storage) - i + 1);
	if (!new_storage)
		return (free(storage), NULL);
	j = 0;
	while (storage[i])
...
char	*fill_storage(int fd, char *storage)
{
	...
	buffer[0] = '\0';
	num_bits = 1;
	while (num_bits > 0 && !ft_gnl_strchr(buffer, '\n'))
	{
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (!storage[fd] || (storage[fd] && !ft_gnl_strchr(storage[fd], '\n')))
